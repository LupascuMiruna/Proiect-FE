...
export class CourseService {
  ...
  getCourses(): Observable<Course[]> {
    return this.http
      ...
      .pipe(catchError(this.handleError<Course[]>('getOrganizations', [])));
  }
  ...
  getCourse(name: string): Observable<Course> {
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError(
          this.handleError<Course>(`getOrganization facultyNmae=${name}`)
        )
      );
  }

  assignUserToCourse(
    userCourse: SignUpToCourseModel
  ): Observable<SignUpToCourseModel> {
    const url = `${this.coursesUrl}/${userCourse.courseId}/Users`;
    console.log(url);
    return this.http
      .post<SignUpToCourseModel>(url, userCourse, {
        headers: this.headers,
      })
      .pipe(
  ...
  removeCourse(name: string): Observable<any> {
    ...
    return this.http
      ...
      .pipe(catchError(this.handleError<string>('removeOrganization')));
  }
